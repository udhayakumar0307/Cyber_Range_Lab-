...
def get_achievements(
    ...
    """
    # Evaluate rules first so any student's earned badges and certificates are dynamically synced
    try:
        from app.services.achievement_manager import achievement_manager
        achievement_manager.evaluate_user_rules(db, current_user.id)
    except Exception as eval_err:
        logger.error(f"Rule evaluation failed in get_achievements for user {current_user.id}: {eval_err}")
    ...
    # Fetch all certificates for the current user to map to lab achievements
    from app.models.certificate import Certificate
    certs = db.query(Certificate).filter(Certificate.user_id == current_user.id).all()
    cert_map = {c.lab_id: c.display_certificate_id for c in certs}
    ...
    for a in all_ach:
        ...
        # Check if this achievement represents a lab completion (prefixed with 'ach-')
        certificate_id = None
        if a.id.startswith("ach-"):
            lab_id = a.id.replace("ach-", "")
            certificate_id = cert_map.get(lab_id)
        else:
            certificate_id = cert_map.get(a.id)
...
def submit_flag(
    ...
    try:
        ...
        # Flush pending changes (score update) to DB so evaluate_user_rules sees new total_score
        db.flush()
...
def get_user_certificates(
    ...
    # Evaluate rules first to auto-issue any achievements/certificates
    try:
        achievement_manager.evaluate_user_rules(db, current_user.id)
    except Exception as eval_err:
        logger.error(f"Rule evaluation failed in get_user_certificates: {eval_err}")
...
def verify_certificate(
    ...
    rule = certificate_service._get_rule_by_lab_id(cert.lab_id)
    
    cert_title = rule.get("cert_title", "Certificate of Completion")
    cert_subtitle_header = rule.get("cert_subtitle_header", "OF COMPLETION")
    cert_subtitle = rule.get("cert_subtitle", "has successfully completed")
    cert_target_name = rule.get("cert_target_name", lab_title)
    cert_description = rule.get("cert_description", "This certificate recognizes successful completion of a practical assessment.")

    return {
        "status": "VALID",
        "display_certificate_id": cert.display_certificate_id,
        "recipient_name": recipient_name,
        "lab_title": cert_target_name,
        "category": lab.category if (lab and lab.category) else "Cyber Security",
        "completion_date": cert.created_at.strftime("%d %b %Y") if cert.created_at else "",
        "duration": f"{hours} Hours",
        "score": user.total_score if user else 100,
        "percentage": 100,
        "badge_earned": f"{cert_target_name} Badge",
        "issued_by": "CyberRange Official Telemetry Platform",
        "pdf_url": cert.pdf_path,
        "png_url": cert.png_path,
        "cert_title": cert_title,
        "cert_subtitle_header": cert_subtitle_header,
