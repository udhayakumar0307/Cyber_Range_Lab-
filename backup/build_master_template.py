...
def build_master():
    ...
    # ── 6. CERTIFICATE / OF COMPLETION — header center ───────────────────────
    f_cert  = load_font("PlusJakartaSans-Bold.ttf", 38)
    f_comp  = load_font("PlusJakartaSans-Bold.ttf", 13)
    cx      = W // 2

    draw_cx(draw, "CERTIFICATE",  f_cert, cx, 22, WHITE)
    draw_cx(draw, "OF  COMPLETION", f_comp, cx, 72, GOLD_LIGHT)
    # Flanking gold lines
    draw.rectangle([cx - 165, 83, cx - 22, 85], fill=GOLD)
    draw.rectangle([cx + 22,  83, cx + 165, 85], fill=GOLD)

    # ── 7. BODY STATIC TEXT ───────────────────────────────────────────────────
    f_certify  = load_font("PlusJakartaSans-Regular.ttf", 16)
    f_complete = load_font("PlusJakartaSans-Regular.ttf", 15)
    f_desc     = load_font("PlusJakartaSans-Regular.ttf", 14)

    # "This is to certify that" — y=152
    draw_cx(draw, "This is to certify that", f_certify, cx, 152, SLATE_600)
    ...
    # "has successfully completed" — y=322
    draw_cx(draw, "has successfully completed", f_complete, cx, 322, SLATE_600)
    ...
    # Description text — static (y=442, y=462)
    desc_1 = "This certificate recognizes successful completion of a CyberRange practical cybersecurity"
    desc_2 = "assessment and confirms demonstrated proficiency in hands-on lab challenges."
    draw_cx(draw, desc_1, f_desc, cx, 442, SLATE_400)
    draw_cx(draw, desc_2, f_desc, cx, 462, SLATE_400)

    # ── 8. METRICS ROW (4 equal columns) ─────────────────────────────────────
    MX_TOP = 492
    MX_BOT = 580

    draw.rectangle([BORDER + 10, MX_TOP, W - BORDER - 10, MX_BOT], fill=(*NAVY, 8))
    draw.rectangle([BORDER + 10, MX_TOP,     W - BORDER - 10, MX_TOP + 2], fill=SLATE_200)
    draw.rectangle([BORDER + 10, MX_BOT - 2, W - BORDER - 10, MX_BOT],     fill=SLATE_200)

    f_metric_lbl = load_font("PlusJakartaSans-Bold.ttf", 11)

    col_xs     = [230, 540, 860, 1170]
    col_labels = ["COMPLETED ON", "DURATION", "SCORE", "ACCURACY"]

    for i, (mcx, lbl) in enumerate(zip(col_xs, col_labels)):
        draw_cx(draw, lbl, f_metric_lbl, mcx, MX_TOP + 12, NAVY_MID)
        # Value placeholder zone
        vw = 200
        draw.rectangle([mcx - vw // 2, MX_TOP + 32, mcx + vw // 2, MX_TOP + 62], fill=OFF_WHITE)
        # Dividers
        if i < len(col_xs) - 1:
            div_x = (col_xs[i] + col_xs[i + 1]) // 2
            draw.rectangle([div_x, MX_TOP + 8, div_x + 1, MX_BOT - 8], fill=SLATE_200)
