...
class GradebookService:
    ...
    def calculate_student_metrics(
        ...
        rubric_result = RubricService.calculate_student_rubric(
            db,
            assignment.id,
            student.id,
        )
        ...
        return {
            ...
            "rubric_criteria_count": len(rubric_result["criteria"]),
            "pending_manual_criteria": rubric_result["pending_manual_criteria"],
        }
    ...
    def _serialize_row(
        ...
        return {
            ...
            "rubric_criteria_count": metrics["rubric_criteria_count"],
            "pending_manual_criteria": metrics["pending_manual_criteria"],
    ...
    def get_gradebook(db: Session, assignment_id: int) -> Dict:
        ...
        RubricService.get_assignment_snapshot(db, assignment_id)
        ...
        avg_rubric = (
            round(sum(row["rubric_percent"] for row in rows) / len(rows), 2)
            if rows
            else 0.0
        )
        avg_final = (
        ...
        return {
            ...
            "summary": {
                ...
                "rubric_name": rows[0]["rubric_name"] if rows else None,
                "pending_manual_criteria": sum(
                    row["pending_manual_criteria"] for row in rows
                ),
    ...
    def save_draft(
        ...
        final_percent = GradebookService._clamp_percent(
            metrics["rubric_percent"] + float(manual_adjustment)
        )
    ...
    def publish(
        ...
        for student in roster:
            ...
            if metrics["pending_manual_criteria"] > 0:
                raise HTTPException(
                    status_code=409,
                    detail=(
                        f"Cannot publish: {student.name or student.email} has "
                        f"{metrics['pending_manual_criteria']} ungraded manual "
            ...
            final_percent = GradebookService._clamp_percent(
                metrics["rubric_percent"] + adjustment
            )
