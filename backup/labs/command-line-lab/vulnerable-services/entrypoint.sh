...
for m in python_module1 python_module2 python_module3 python_module4 python_module5; do
    hash=$(gen_flag "$m")
    printf -v "FLAG_${m}" "FLAG{cll_%s_%s_%s}" "$m" "$STUDENT_ID" "$hash"
done

# C track modules
for m in c_module1 c_module2 c_module3 c_module4 c_module5; do
    hash=$(gen_flag "$m")
    printf -v "FLAG_${m}" "FLAG{cll_%s_%s_%s}" "$m" "$STUDENT_ID" "$hash"
done

# C++ track modules
for m in cpp_module1 cpp_module2 cpp_module3 cpp_module4 cpp_module5; do
    hash=$(gen_flag "$m")
    printf -v "FLAG_${m}" "FLAG{cll_%s_%s_%s}" "$m" "$STUDENT_ID" "$hash"
done
...
cat > /flags/flags.json <<EOF
{
  ...
  "flags": {
    ...
    "cpp_module1": "${FLAG_cpp_module1}",
    "cpp_module2": "${FLAG_cpp_module2}",
    "cpp_module3": "${FLAG_cpp_module3}",
    "cpp_module4": "${FLAG_cpp_module4}",
    "cpp_module5": "${FLAG_cpp_module5}"
  }
}
EOF
...
# ── Python Track Scaffolding ──
for i in 1 2 3 4 5; do
    DXS mkdir -p /home/student/module${i}/python
done
DXS bash -c "echo 'Welcome to Python File I/O' > /home/student/module4/python/data.txt"

# ── C Track Scaffolding ──
for i in 1 2 3 4 5; do
    DXS mkdir -p /home/student/module${i}/c
done

# ── C++ Track Scaffolding ──
for i in 1 2 3 4 5; do
    DXS mkdir -p /home/student/module${i}/cpp
done
