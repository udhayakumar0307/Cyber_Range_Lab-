    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs">
      <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-xl w-full border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none"
              />
            </div>

            <button
              onClick={() => setIsAddingUser(!isAddingUser)}
              className="px-3 py-1.5 rounded-lg bg-[#0052CC] hover:bg-blue-600 text-white font-bold text-xs inline-flex items-center gap-1.5 shadow-xs transition-colors cursor-pointer"
            >
          ...
          {isAddingUser && (
            <form onSubmit={handleAddMember} className="p-4 bg-blue-50/70 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-800 rounded-xl space-y-3">
              <h4 className="text-xs font-bold text-[#0052CC] dark:text-blue-400">Quick Assign User to {group.name}</h4>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <input
                  ...
                  className="p-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md text-slate-800 dark:text-slate-100 focus:outline-none"
                />
                <input
                  type="email"
              ...
              <div className="flex justify-end gap-2 text-xs">
                <button
                  ...
                  className="px-3 py-1 text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 font-bold"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-3 py-1 bg-[#0052CC] text-white rounded-md font-bold hover:bg-blue-600 cursor-pointer"
                >
                  Save to Cohort
                </button>
              </div>
