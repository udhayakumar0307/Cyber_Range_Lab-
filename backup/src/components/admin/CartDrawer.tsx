      ...
      <div className="fixed top-0 right-0 bottom-0 z-50 w-[500px] bg-[#0d1117] border-l border-slate-700/60 shadow-2xl flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-700/60 bg-[#0d1117]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-black text-white">Lab Cart</h2>
          ...
          {safeItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-center">
              <div className="p-5 bg-slate-800/60 border border-slate-700/60 rounded-2xl">
                ...
                <p className="text-xs text-slate-500 mt-1">Add a lab from the marketplace</p>
              </div>
            </div>
          ) : (
            safeItems.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="bg-slate-800/50 border border-slate-700/60 rounded-2xl p-4 space-y-4"
                >
                  {/* Lab name + remove */}
