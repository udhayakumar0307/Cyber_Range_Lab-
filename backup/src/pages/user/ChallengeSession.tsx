...
const RECON_MODULES: ReconModule[] = [
  {
    id: 'module1',
    title: 'Module 1: FTP Service Enumeration & Flag Discovery',
    points: 100,
    description: 'Find the open FTP port on the target machine 10.10.0.10, log in anonymously, and locate the Module 1 flag.',
    mission: 'Perform port discovery using Nmap to find the active FTP server. Connect anonymously to download and read the public README and discover the flag file.',
    objectives: [
      {
        label: "Scan target ports using Nmap: `nmap 10.10.0.10`",
        triggerPatterns: [/nmap\b/i],
      },
      {
        label: "Connect anonymously to the FTP server on port 21.",
        triggerPatterns: [/ftp\b/i],
      },
      {
        label: "Retrieve the Module 1 flag from `/pub/FLAG1_DISCOVERED.txt`.",
        triggerPatterns: [/cat.*FLAG1_DISCOVERED/i, /get.*FLAG1_DISCOVERED/i],
      },
    ],
    hints: [
      "Use `nmap 10.10.0.10` in your terminal to verify port 21 is open.",
      "Run `ftp 10.10.0.10`, log in with username 'anonymous' and empty password, then check the 'pub' directory.",
    ],
  },
  {
    id: 'module2',
    title: 'Module 2: MySQL Database Access & Credential Harvesting',
    points: 150,
    description: 'Connect to the MySQL database on port 3306 with the scanner user and harvest user credentials and flags.',
    mission: 'Establish a connection to the MariaDB server. Retrieve the employee credentials and extract the Module 2 flag stored in the administrator\'s secret note.',
    objectives: [
      {
        label: "Connect to the MySQL database using client tools (use --skip-ssl).",
        triggerPatterns: [/mysql\b/i],
      },
      {
        label: "Access the database and view the `employees` table.",
        triggerPatterns: [/select.*employees/i, /show tables/i, /mysql\b/i],
      },
      {
        label: "Retrieve the administrator's secret note containing the Module 2 flag.",
        triggerPatterns: [/select.*employees/i, /select.*secret_note/i],
      },
    ],
    hints: [
      "Connect using `mysql -h 10.10.0.10 -u scanner --skip-ssl`.",
      "Run `use techcorp_db;` and query the tables using `select * from employees;`.",
    ],
  },
  {
    id: 'module3',
    title: 'Module 3: REST API Interaction & Key Retrieval',
    points: 200,
    description: 'Retrieve the internal API key from the database and query the internal API running on port 9000.',
    mission: 'Examine the system configuration table in the database to retrieve the API key and query the internal service API on port 9000 to find the Module 3 flag.',
    objectives: [
      {
        label: "Query the `config` table in the database to find the internal API key.",
        triggerPatterns: [/select.*config/i, /select.*key_name/i],
      },
      {
        label: "Interact with the REST API on port 9000 using curl.",
        triggerPatterns: [/curl\b/i, /http:\/\/10\.10\.0\.10:9000/i],
      },
      {
        label: "Retrieve the Module 3 flag from the API response.",
        triggerPatterns: [/curl\b/i, /FLAG/i],
      },
    ],
    hints: [
      "Query the config table using `select * from config;`.",
      "Use `curl http://10.10.0.10:9000` or inspect the API endpoints.",
    ],
  },
  {
    id: 'module4',
    title: 'Module 4: SSH System Access & Privilege Auditing',
    points: 250,
    description: 'Use the discovered backup credentials to SSH into the target system on port 22 and retrieve the hidden backup flag.',
    mission: 'Log in as the backup user via SSH using the credentials harvested from the database. Locate the hidden backup key file in the home directory.',
    objectives: [
      {
        label: "SSH into the target system as the `backup` user on port 22.",
        triggerPatterns: [/ssh\b/i, /ssh.*backup/i],
      },
      {
        label: "Locate and read the secret backup file `/home/backup/.secret_backup_key`.",
        triggerPatterns: [/cat.*\.secret_backup_key/i, /cat.*secret/i],
      },
    ],
    hints: [
      "Run `ssh backup@10.10.0.10` and use the password `backup123`.",
      "Check the home directory of the backup user using `ls -la` and `cat .secret_backup_key`.",
    ],
  },
  {
    id: 'module5',
    title: 'Module 5: Admin Console Exploitation (Capstone)',
    points: 300,
    description: 'Analyze the admin console credentials referenced in the SSH configuration files and exploit the admin service on port 8888.',
    mission: 'Inspect the SSH configurations and identity notes left in the backup home folder. Connect to the Admin Console on port 8888, authenticate, and retrieve the final capstone flag.',
    objectives: [
      {
        label: "Inspect the SSH config files `/home/backup/.ssh/config` and `/home/backup/.ssh/id_rsa.notes`.",
        triggerPatterns: [/cat.*config/i, /cat.*id_rsa\.notes/i, /ls/i],
      },
      {
        label: "Connect to the Admin Console running on port 8888 using netcat.",
        triggerPatterns: [/nc\b/i, /netcat\b/i, /telnet\b/i],
      },
      {
        label: "Authenticate and run the FLAG command to retrieve the capstone flag.",
        triggerPatterns: [/AUTH\b/i, /FLAG\b/i],
      },
    ],
    hints: [
      "Use `cat ~/.ssh/config` and `cat ~/.ssh/id_rsa.notes` to discover the admin console port and credentials.",
      "Connect using `nc 10.10.0.10 8888`, type `AUTH admin techcorp_admin_2024`, and then run the `FLAG` command.",
    ],
  },
];
